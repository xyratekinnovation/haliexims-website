...
export function ProductGallery({ title, images }: ProductGalleryProps) {
  ...
  return (
    <div>
      <div className="group relative overflow-hidden rounded-3xl border border-border bg-surface aspect-[4/3]">
        <CatalogImg
          kind="product"
          src={current}
          alt={title}
          className="h-full w-full object-cover transition-transform duration-700 ease-[cubic-bezier(0.2,0.8,0.2,1)] group-hover:scale-110"
          width={1200}
          height={900}
        />
      ...
      {images.length > 1 && (
        <div className="mt-3 flex gap-3 overflow-x-auto pb-1">
          {images.map((src, i) => (
            ...
            >
              <CatalogImg kind="product" src={src} alt="" className="h-full w-full object-cover" loading="lazy" />
            </button>
          ))}
        </div>
      )}
